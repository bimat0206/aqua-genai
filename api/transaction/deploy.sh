...
# Deploy script for Transaction API Lambda Function
# This script builds the Docker image and pushes it to ECR

set -e

# Configuration
FUNCTION_NAME="transaction-api"
...
get_ecr_repository() {
    ...
    # Get the repository name that contains 'transaction'
    REPO_NAME=$(aws ecr describe-repositories --region $AWS_REGION --query "repositories[?contains(repositoryName, 'transaction')].repositoryName" --output text 2>/dev/null | tr '\t' '\n' | head -1)

    if [ -z "$REPO_NAME" ]; then
        log_error "ECR repository not found. Please ensure Terraform has been applied and the ECR repository exists."
        log_info "Expected repository name pattern: *transaction*"
...
build_and_test() {
    ...
    if ls *_test.go 1> /dev/null 2>&1; then
        AWS_DATASET_BUCKET=test-bucket AWS_REGION=us-east-1 go test -v > /dev/null 2>&1
    else
...
update_lambda() {
    ...
    LAMBDA_FUNCTION_NAME=$(aws lambda list-functions --query "Functions[?contains(FunctionName, 'transaction')].FunctionName" --output text 2>/dev/null | head -1)

    if [ -z "$LAMBDA_FUNCTION_NAME" ]; then
        log_error "Lambda function not found. Please ensure Terraform has been applied and the Lambda function exists."
        log_info "Expected function name pattern: *transaction*"
...
test_function() {
    ...
    # Create test payload file for transaction API
    cat > test_payload.json << 'EOF'
{
  "httpMethod": "GET",
  "path": "/api/transaction/12345678-1234-1234-1234-123456789abc",
...
deploy() {
    log_info "Starting deployment of Transaction API Lambda Function..."
    ...
    log_info "The Transaction API Lambda function is now deployed and ready to use."
    log_info "Endpoint: GET /api/transaction/{transactionId}"
    log_info "Features: Transaction details retrieval with verification results and image access"
    log_info "Parameters: transactionId (UUID) - required path parameter"
}
...
go_run() {
    ...
    log_info "  export AWS_RESULT_TABLE=your-dynamodb-table"
    log_info "  export AWS_DATASET_BUCKET=your-dataset-bucket"
    log_info "  export AWS_IMPUT_IMG_VALIDATION_BUCKET=your-validation-bucket"
...
case "${1:-deploy}" in
    ...
    "help"|"-h"|"--help")
        ...
        echo "  AWS_RESULT_TABLE                DynamoDB table for verification results"
        echo "  AWS_DATASET_BUCKET              S3 bucket containing dataset images"
        echo "  AWS_IMPUT_IMG_VALIDATION_BUCKET S3 bucket for uploaded validation images"
        echo "  LOG_LEVEL                       Log level (default: INFO)"
        echo ""
        echo "Features:"
        echo "  - Transaction details retrieval by UUID"
        echo "  - Comprehensive verification results parsing"
        echo "  - Presigned URL generation for image access"
        echo "  - AI analysis data and cost estimation"
