...
# Deploy script for Catalog API Lambda Function
...
get_ecr_repository() {
    ...
    # Get the repository name that contains 'catalog'
    REPO_NAME=$(aws ecr describe-repositories --region $AWS_REGION --query "repositories[?contains(repositoryName, 'catalog')].repositoryName" --output text 2>/dev/null | tr '\t' '\n' | head -1)

    if [ -z "$REPO_NAME" ]; then
        log_error "ECR repository not found. Please ensure Terraform has been applied and the ECR repository exists."
        log_info "Expected repository name pattern: *catalog*"
...
update_lambda() {
    ...
    LAMBDA_FUNCTION_NAME=$(aws lambda list-functions --query "Functions[?contains(FunctionName, 'catalog')].FunctionName" --output text 2>/dev/null | head -1)

    if [ -z "$LAMBDA_FUNCTION_NAME" ]; then
        log_error "Lambda function not found. Please ensure Terraform has been applied and the Lambda function exists."
        log_info "Expected function name pattern: *catalog*"
...
test_function() {
    ...
    # Create test payload file for catalog API
...
deploy() {
    log_info "Starting deployment of Catalog API Lambda Function..."
    ...
    log_info "The Catalog API Lambda function is now deployed and ready to use."
    log_info "Endpoint: GET /api/catalog"
    log_info "Features: S3 dataset catalog discovery and image presigned URLs"
    log_info "Parameters: type (categories|products|images), category, productId"
}
...
go_run() {
    ...
    log_info "  export PRESIGNED_URL_EXPIRY=15"
...
case "${1:-deploy}" in
    ...
    "help"|"-h"|"--help")
        ...
        echo "  PRESIGNED_URL_EXPIRY Presigned URL expiry in minutes (default: 15)"
        echo "  LOG_LEVEL           Log level (default: INFO)"
        echo ""
        echo "Features:"
        echo "  - S3 dataset catalog discovery"
        echo "  - Category and product listing"
        echo "  - Image presigned URL generation"
