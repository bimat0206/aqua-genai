...
# Deploy script for History API Lambda Function
# This script builds the Docker image and pushes it to ECR

set -e

# Configuration
FUNCTION_NAME="history-api"
...
get_ecr_repository() {
    ...
    # Get the repository name that contains 'history'
    REPO_NAME=$(aws ecr describe-repositories --region $AWS_REGION --query "repositories[?contains(repositoryName, 'history')].repositoryName" --output text 2>/dev/null | tr '\t' '\n' | head -1)

    if [ -z "$REPO_NAME" ]; then
        log_error "ECR repository not found. Please ensure Terraform has been applied and the ECR repository exists."
        log_info "Expected repository name pattern: *history*"
...
update_lambda() {
    ...
    LAMBDA_FUNCTION_NAME=$(aws lambda list-functions --query "Functions[?contains(FunctionName, 'history')].FunctionName" --output text 2>/dev/null | head -1)

    if [ -z "$LAMBDA_FUNCTION_NAME" ]; then
        log_error "Lambda function not found. Please ensure Terraform has been applied and the Lambda function exists."
        log_info "Expected function name pattern: *history*"
...
test_function() {
    ...
    # Create test payload file for history API
...
deploy() {
    log_info "Starting deployment of History API Lambda Function..."
    ...
    log_info "The History API Lambda function is now deployed and ready to use."
    log_info "Endpoint: GET /api/history"
    log_info "Features: Verification history, search, filtering, and export functionality"
    log_info "Parameters: page, limit, search, status, startDate, endDate, format (for export)"
}
...
go_run() {
    ...
    log_info "  export EXPORT_BUCKET=your-export-bucket"
...
case "${1:-deploy}" in
    ...
    "help"|"-h"|"--help")
        ...
        echo "  AWS_DATASET_BUCKET   S3 bucket containing dataset"
        echo "  EXPORT_BUCKET        S3 bucket for export files"
        echo "  LOG_LEVEL           Log level (default: INFO)"
        echo ""
        echo "Features:"
        echo "  - Verification history listing with pagination"
        echo "  - Advanced search and filtering capabilities"
        echo "  - Export functionality (CSV, JSON, PDF)"
